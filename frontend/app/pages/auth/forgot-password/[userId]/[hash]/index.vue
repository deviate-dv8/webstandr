...
async function resetPassword() {
	...
	errMessage.value = "";
	try {
		const response = await $fetch(`${API}/api/email/reset-password/${userId}/${hash}`, {
			method: 'POST',
			body: {
		...
		<section class="min-h-[calc(100svh-60px)] flex items-center justify-center">
			<div v-if="!error" class="container flex flex-col items-center justify-center px-6 mx-auto">
				...
				<div class="w-full max-w-sm mt-4">
					<input v-model="resetPasswordCredentials.password"
				...
				<div class="w-full mt-4 relative max-w-sm">
					<input v-model="resetPasswordCredentials.confirmPassword"
						class="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-500 bg-white border rounded-lg focus:border-orange-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-orange-300"
						:type="showPassword ? 'text' : 'password'" placeholder="Confirm Password" aria-label="Confirm Password"
						required>
					<button v-if="!showPassword" type="button" class="absolute right-4 top-5 hover:text-gray-500 duration-100"
						@click="showPassword = true">
						<Icon name="weui:eyes-on-filled" class="text-xl" />
					</button>
					<button v-else type="button" class="absolute right-4 top-5 hover:text-gray-500 duration-100"
